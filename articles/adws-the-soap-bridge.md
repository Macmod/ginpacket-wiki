...
3. **SASL** ([RFC 4422](https://datatracker.ietf.org/doc/html/rfc4422) / [MS-ADTS](https://winprotocoldoc.z19.web.core.windows.net/MS-ADTS/[MS-ADTS].pdf)) is the protocol-specific wrapper the LDAP client uses to tell the server what it'll authenticate with:
   ...
   * **GSSAPI** - this SASL mechanism is supposed to deviate from the original meaning of GSSAPI and be **Kerberos only** (according to [RFC 4752](https://datatracker.ietf.org/doc/html/rfc4752) and MS-ADTS), but **that's not true** for AD - in practice a client can simply embed either NTLM or Kerberos inside GSSAPI and AD will accept it (without mechanism negotiation);
...
{% hint style="info" %}
Integrity and confidentiality are properties provided by the usage of **signing** ("using a signature for to make sure payloads were not tampered with") and **sealing** ("encrypting payloads to prevent snooping by 3rd parties")
{% endhint %}
...
As much as these questions are interesting to think about (I won't tell you all the answers 🙂), mostly they only teach us one lesson - different tools may authenticate in different ways under the wire, **even when provided the same authentication material**. As a matter of fact, if we were to consider combinations of methods/mechanisms, signing and sealing, there are at least 23 ways to authenticate to LDAP on Windows DCs; considering the allowed variants over different transport layers (plain `ldap` or `ldap+starttls` or `ldaps`), that number grows to 37. And we haven't even considered ciphersuite variations, so keep that in mind the next time you're struggling with authentication - it's not supposed to be easy 🫠.
...
sequenceDiagram
    ...
    Note over C,S: NNS handshake happens next
    end
```
